...
<p class="div-no">1교시</p>

## Claude Code

<p class="div-sub">무엇인지, 어디서 도는지, 화면 어디를 보는지</p>

<p class="div-file">1일차 · 도구</p>

---
class: top-led brand-cc
---

<p class="eyebrow">CLAUDE CODE · 무엇인가</p>
...
<p class="eyebrow">CLAUDE CODE · CLI vs 데스크톱</p>

# 같은 앱입니다. <em>인터페이스만 다릅니다</em>

| | CLI | 데스크톱 앱 |
|---|---|---|
| 권한 모드 | `dontAsk` 포함 전부 | Manual · Accept edits · Plan · Auto (Bypass 는 설정에서 켬) |
| MCP 연결 | 설정 파일로 | **커넥터 UI** 또는 설정 파일 |
| 플러그인 | `/plugin` 명령 | `/plugin` + **플러그인 관리 화면** |
| 파일 첨부 | 없음 | **이미지 · PDF** |
| `@` 파일 언급 | 글자로 입력 | **자동완성** |
| 세션 격리 | `--worktree` 플래그 | 세션 UI |

<p class="src"><a href="https://code.claude.com/docs/en/desktop">code.claude.com/docs/en/desktop</a> · Feature comparison</p>

<p class="lead">엔진이 같으니 <em>결과도 같습니다.</em> 다른 건 무엇이 눈에 보이느냐입니다.</p>

<p class="note"><b>버전 주의</b> 에이전트 팀 같은 새 기능은 <em>특정 버전 이상</em>에서만 켜집니다. 옆사람 화면에 있는 게 내 화면에 없으면 대개 버전 차이입니다.</p>
...
<p class="eyebrow">CLAUDE CODE · 노력 수준</p>

# 얼마나 <em>공들여 답할지</em>를 고릅니다

<div class="flow">
<div><b>low</b><span>짧고 빠르게</span></div>
<div><b>medium</b><span>기본</span></div>
<div><b>high</b><span>더 따져본다</span></div>
<div><b>xhigh</b><span>오래 걸린다</span></div>
<div><b>ultracode</b><span>최대치</span></div>
</div>

<p class="lead">높일수록 더 철저해지지만 <em>시간이 더 걸리고 한도를 더 빨리 씁니다.</em></p>

<div class="deflist narrow">
<div><b>바꾸는 법</b><span><code>/effort</code> 로 고르거나, <code>claude --effort ultracode</code> 로 시작</span></div>
<div><b>기본은 medium</b><span>대부분의 일은 여기서 됩니다. 막힐 때만 올립니다</span></div>
</div>

<p class="src"><a href="https://code.claude.com/docs/en/model-config">code.claude.com/docs/en/model-config</a> · Adjust effort level</p>

---
class: top-led brand-cc
---

<p class="eyebrow">CLAUDE CODE · 여럿 굴리기</p>

# 결과만 받을 것인가, <em>서로 이야기하게 할 것인가</em>

<div class="split evidence">
<div>

<p class="quote">서브에이전트는 결과를 본체에 보고합니다. 에이전트 팀에서는 팀원들이 할 일 목록을 공유하고, 일을 집어 가고, <em>서로 직접 대화합니다.</em><span class="who">Claude Code 공식 문서 · Agent teams</span></p>

<div class="deflist narrow">
<div><b>서브에이전트</b><span>결과만 중요할 때. 토큰이 덜 든다</span></div>
<div><b>에이전트 팀</b><span>의논이 필요할 때. 각자 별도 인스턴스라 더 든다</span></div>
</div>
</div>
<figure class="shot mark-none" data-origin="web" data-source="https://code.claude.com/docs/en/agent-teams">
<img src="https://mintcdn.com/claude-code/nsvRFSDNfpSU5nT7/images/subagents-vs-agent-teams-light.png?w=1100&fit=max&auto=format&n=nsvRFSDNfpSU5nT7&q=85&s=923986caa23c0ef2c27d7e45f4dce6d1" alt="서브에이전트는 본체 하나에 결과를 되돌려 주는 방사형 구조이고, 에이전트 팀은 팀원들이 서로 직접 메시지를 주고받는 그물형 구조로 그려져 있다" />
<figcaption>공식 문서 그림 · <a href="https://code.claude.com/docs/en/agent-teams">code.claude.com/docs/en/agent-teams</a></figcaption>
</figure>
</div>

<p class="note"><b>이 수업에서는</b> 서브에이전트까지만 씁니다. 팀은 「이런 것도 있다」로 한 번 보여드립니다.</p>
...
<p class="div-sub">다섯 가지를 직접 눌러봅니다. 코드는 아직 없습니다</p>

<p class="div-file">1일차 · 1교시</p>

---
class: top-led brand-cc
---

<p class="eyebrow">실습 1-1 · 도구에게 도구를 묻기</p>

# 첫 질문은 <em>「너 뭐 할 수 있어?」</em>

<div class="split visual-dominant">
<div>
<p class="thesis">설명서를 읽는 것보다 빠릅니다. <em>지금 이 세션 기준</em>으로 답합니다.</p>

```text
Claude Code는 뭘 할 수 있어?
```

<p class="note"><b>왜 이걸 먼저 하나</b> 사람마다 켜져 있는 것이 다릅니다. 스킬·MCP·플러그인이 붙어 있으면 그것까지 포함해서 답합니다.</p>
</div>
<figure class="shot lab" data-origin="capture">
<img src="./images/lab/q1-crop.png" alt="Claude Code가 코드 작업, 실행 환경, 확장, 산출물 네 묶음으로 자기가 할 수 있는 일을 정리해 답한 화면" />
<figcaption>코드 작업 · 실행 환경 · 확장 · 산출물 네 묶음으로 답합니다</figcaption>
</figure>
</div>
...
<p class="eyebrow">실습 1-2 · 모르는 말 되묻기</p>

# 모르는 단어가 나오면 <em>거기서 멈추지 않습니다</em>

<div class="split visual-dominant">
<div>
<p class="thesis">답에 <code>cron</code>·<code>/loop</code>·Artifact 같은 말이 나옵니다. 그대로 되물으면 됩니다.</p>

```text
스케줄에서 cron이랑 loop 차이는 뭐야?
```

<p class="note"><b>표로 답하게 하려면</b> 「비교해서 표로」라고 붙이면 됩니다. 두 개를 나란히 놓는 순간 차이가 보입니다.</p>
</div>
<figure class="shot lab" data-origin="capture">
<img src="./images/lab/q2-crop.png" alt="cron과 loop를 실행 주체·지속성·컨텍스트·주기·용도 다섯 축으로 비교한 표와, Artifact 공유 링크에 대한 설명" />
<figcaption>실행 주체 · 지속성 · 컨텍스트 · 주기 · 용도로 갈라 답했습니다</figcaption>
</figure>
</div>
...
<p class="eyebrow">실습 1-3 · 곁가지 질문</p>

# 본 대화를 흐트리지 않고 <em>옆에서 묻기</em>

<div class="split">
<figure class="shot lab" data-origin="capture">
<img src="./images/lab/q3-menu-crop.png" alt="답변 본문에서 한 줄을 드래그하자 사이드 채팅으로 보내기와 답글 두 버튼이 떠 있는 화면" />
<figcaption>① 모르는 대목을 <em>드래그</em>하면 두 버튼이 뜹니다</figcaption>
</figure>
<figure class="shot lab mark-ok" data-origin="capture">
<img src="./images/lab/q3-chat-crop.png" alt="오른쪽에 사이드 채팅 창이 열려 배포와 CI가 무엇인지 따로 설명하고 있고, 왼쪽 본 대화는 그대로 남아 있다" />
<figcaption>② 오른쪽에서 따로 답합니다. 본 대화는 그대로입니다</figcaption>
</figure>
</div>

<p class="lead">「배포가 뭐예요?」를 묻겠다고 <em>본 작업을 끊지 않습니다.</em> 터미널에서는 <code>/btw</code> 로 같은 일을 합니다.</p>

---
class: top-led brand-cc
---

<p class="eyebrow">실습 1-4 · 답글과 검색</p>

# 확실치 않다고 하면 <em>찾아보라고 시킵니다</em>

<div class="split visual-dominant">
<div>
<p class="thesis">앞 답에서 「제가 단정하기 어렵습니다」라고 한 대목을 드래그해 <em>답글</em>로 되묻습니다.</p>

```text
검색해서 현재 적용 기준으로 알려줘
```

<p class="note"><b>이 습관이 중요합니다</b> 모델은 학습 시점 이후를 모릅니다. 「모르겠다」가 나오면 그게 <em>검색을 시킬 자리</em>입니다.</p>
</div>
<figure class="shot lab mark-ok" data-origin="capture">
<img src="./images/lab/q4-crop.png" alt="웹 검색 두 번과 공식 문서 두 건을 가져온 뒤, 플랜별 공유 방식을 표로 확정해 답한 화면" />
<figcaption>공식 문서 두 건을 가져와 <em>플랜별로</em> 확정해 답했습니다</figcaption>
</figure>
</div>
...
<p class="eyebrow">실습 1-5 · 산출물</p>

# 대화를 <em>한 장의 웹페이지로</em>

<div class="split visual-dominant">
<div>
<p class="thesis">지금까지 물어본 것을 정리해 <em>링크로 남깁니다.</em></p>
...
<div class="deflist narrow">
<div><b>기본은 비공개</b><span>발행 직후엔 본인만 보입니다</span></div>
<div><b>같은 파일 = 같은 URL</b><span>다시 발행하면 링크는 그대로, 내용만 갱신</span></div>
</div>
</div>
<figure class="shot lab mark-ok" data-origin="capture">
<img src="./images/lab/q5-crop.png" alt="claude-code-qa.html이 생성되고 Claude Code 세션 문답이라는 제목으로 발행되었다는 화면" />
<figcaption>HTML 한 장이 만들어지고 공유 링크가 생깁니다</figcaption>
</figure>
</div>

<p class="src"><a href="https://code.claude.com/docs/en/artifacts">code.claude.com/docs/en/artifacts</a></p>

---
class: top-led brand-cc
---

<p class="eyebrow">CLAUDE CODE · 화면 제어</p>

# 화면을 <em>안 건드리는 방법부터</em> 고릅니다

<div class="steps">
<div><b>MCP 커넥터</b><span>Gmail · 캘린더 · Slack 등. API 로 처리해 <em class="c-ok">화면을 전혀 안 씁니다</em></span></div>
<div><b>인앱 브라우저</b><span>Claude 앱 안의 별도 창. 내 마우스나 다른 앱을 건드리지 않습니다</span></div>
<div><b>Chrome 확장</b><span>내 Chrome 의 별도 탭에서 DOM 으로. 탭이 왔다갔다하는 건 보입니다</span></div>
<div><b>컴퓨터 제어</b><span>실제 마우스·키보드를 움직입니다. <em class="bad">진짜 백그라운드는 불가능합니다</em></span></div>
</div>

<p class="lead">공식 문서가 정한 순서입니다 — <em>어느 것도 아닐 때</em> 컴퓨터 제어를 씁니다.</p>

<p class="src"><a href="https://code.claude.com/docs/en/desktop">code.claude.com/docs/en/desktop</a> · When computer use applies</p>

---
class: top-led brand-cc
---

<p class="eyebrow">실습 2 · 컴퓨터 제어</p>

# 마우스를 <em>실제로 움직여 봅니다</em>

<div class="split">
<div>
<p class="thesis">앞의 네 단계 중 <em>마지막</em>을 직접 봅니다. 왜 마지막인지 몸으로 알게 됩니다.</p>
...
<p class="note"><b>먼저 허용을 묻습니다</b> 어떤 앱을 만질지 목록을 보여주고 승인받습니다. 승인한 앱만 만집니다.</p>
</div>
<div>

<div class="deflist narrow">
<div><b>화면 점유</b><span>실행하는 동안 다른 일을 못 합니다</span></div>
<div><b>브라우저</b><span>읽기만 됩니다. 클릭·입력은 막혀 있습니다</span></div>
<div><b>터미널·편집기</b><span>클릭만 됩니다. 타이핑은 막혀 있습니다</span></div>
</div>

<p class="lead">그래서 실무에서는 <em>커넥터가 있으면 커넥터를</em> 씁니다.</p>
</div>
</div>
...
<p class="div-no">2교시</p>

## Antigravity

<p class="div-sub">에이전트를 여러 개 굴리는 자리, 그리고 계획을 받아 보는 법</p>
...
<p class="eyebrow">ANTIGRAVITY · 무엇인가</p>
